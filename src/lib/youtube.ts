// YouTube API v3 Integration Library
...
function getYouTubeConfig(): YouTubeConfig {
  ...
  return requiredEnvVars as YouTubeConfig;
}
 ...
 * @param config - Video metadata and configuration
 * @returns Promise with video ID and metadata
 */
 ...
 */
export async function getVideoMetadata(videoId: string): Promise<YouTubeVideoData | null> {
  try {
    const { youtube } = await createYouTubeClient();
    ...
    return {
      ...
      privacy: (video.status?.privacyStatus as 'public' | 'unlisted' | 'private') || 'public'
    };
  } catch (error) {
 ...
 * @param videoData - Video metadata to save
...
export async function loadAllVideoMetadata(): Promise<YouTubeVideoData[]> {
  ...
  try {
    ...
    const videoDataPromises = jsonFiles.map(async (file) => {
      ...
      return JSON.parse(content) as YouTubeVideoData;
    });
